...
> **提示**：下载中心里 **Server 包**、**Client 包**、**桌面 GUI** 是三套独立安装包，按角色分别下载，不要搞混。

## 3. 部署服务端（公网机器）

1. 从下载中心获取对应系统的 **Server 包**，解压得到 `KunTunetServer`。
...
从下载中心获取对应系统的 **Client 包**，解压得到 `KunTunetClient`。假设内网 Web 在 `127.0.0.1:8080`，要映射到公网 `9000` 端口：
