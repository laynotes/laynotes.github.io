...
> **提示**：下载中心里 **命令行一体部署包**（内含 Server / Client / Admin）与 **桌面 GUI** 是两套安装包。一体包解压后按角色取用对应二进制即可。

## 3. 部署服务端（公网机器）

1. 从下载中心获取对应系统的 **命令行一体部署包**，解压后取用 `KunTunetServer`。
...
从下载中心获取对应系统的 **命令行一体部署包**，解压后取用 `KunTunetClient`。假设内网 Web 在 `127.0.0.1:8080`，要映射到公网 `9000` 端口：
